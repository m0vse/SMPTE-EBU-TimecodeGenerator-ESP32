...
form{display:grid;gap:12px}label{font-size:12px;color:var(--muted)}input{width:100%;margin-top:5px;padding:11px 12px;border:1px solid var(--line);border-radius:9px;background:#091522;color:var(--text);outline:none}input:focus{border-color:var(--cyan);box-shadow:0 0 0 3px #31d7e818}.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}button{border:0;border-radius:9px;padding:11px 14px;background:linear-gradient(135deg,#22bfd1,#4e6fff);color:white;font-weight:700;cursor:pointer}button.secondary{background:#1b2b41;border:1px solid #30455f}button.danger{background:#4b2430;border:1px solid #7c3446}button:disabled{opacity:.55;cursor:wait}.actions{display:flex;gap:9px;flex-wrap:wrap}.notice{min-height:22px;margin-top:10px;font-size:12px;color:var(--green)}footer{text-align:center;color:#607792;font-size:11px;margin-top:24px}
...
<section class="card wide"><h2>Clock configuration</h2><form id="clockForm"><label>POSIX timezone<input id="timezone" name="timezone" maxlength="128" required></label><label>Output adjustment in seconds<input id="fiddle" name="fiddle" type="number" min="-3600" max="3600" step="0.001" required></label><button>Save clock settings</button></form><div id="clockNotice" class="notice"></div></section>
