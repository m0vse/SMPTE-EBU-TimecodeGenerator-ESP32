...
void setTS(unsigned char _hour, unsigned char _min, unsigned char _sec) {
  debugI("Old BCD Time %02x:%02x:%02x.%02x", hourCount, minsCount, secsCount, frameCount);
  hourCount = BCD(_hour);
  secsCount = BCD(_sec);
  minsCount = BCD(_min);
  debugI("New BCD Time %02x:%02x:%02x.%02x", hourCount, minsCount, secsCount, frameCount);
};
