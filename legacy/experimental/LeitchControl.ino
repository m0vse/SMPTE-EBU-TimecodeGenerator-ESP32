...
extern volatile bool nmea_active;
...
TaskHandle_t xTelnetHandle = NULL;
...
void setup() {
  ...
  // Create the Mutex before starting any other task.
  xWiFi = xSemaphoreCreateMutex();

  xTaskCreate(TaskRMT, "TaskRMT", 4096, NULL, 3, &xRMTHandle);
  xTaskCreate(TaskOTA, "TaskOTA", 2048, NULL, 0, &xOTAHandle);
  xTaskCreate(TaskWEB, "TaskWEB", 4096, NULL, 0, &xWEBHandle);
  xTaskCreate(TaskTelnet, "TaskTelnet", 4096, NULL, 0, &xTelnetHandle);
...
void TaskNTP( void *pvParameters )
{
  ...
  for (;;) // A Task shall never return or exit.
  {
    ...
    if (nmea_active) {
      ...
      if (!ntp_active && timeStatus()==timeSet && isset != minute()){
        ntp_active=true;
        _setTS(hour(), minute(), second(),(ms()*FPS)/1000);
        isset=minute();
      }
